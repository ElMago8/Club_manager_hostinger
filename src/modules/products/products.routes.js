...
const nullText = z.preprocess((v) => (typeof v === "string" && v.trim() === "" ? null : v), z.string().trim().nullable().optional());
// ─── Schemas ─────────────────────────────────────────────────────────────────
...
function productoRoutes() {
    const router = Router();
    const include = { categoria: { select: { id: true, nombre: true, codigoCategoria: true } } };
    router.get("/", async (req, res, next) => {
        try {
            const { estado, tipoProducto, categoriaId } = req.query;
...
function loteProductoRoutes() {
    ...
    router.get("/", async (req, res, next) => {
        try {
            const { estado, productoId, geneticaId, ubicacionId } = req.query;
    ...
    router.get("/:id", async (req, res, next) => {
        try {
            ...
            if (!record)
                throw new ApiError(404, "Lote de producto no encontrado.");
            res.json(record);
        }
        catch (e) {
            next(e);
