...
function crudRoutes(delegate, createSchema, updateSchema = createSchema.partial()) {
    const router = Router();
...
function activePlantWhereForCamilla(camillaId) {
    return {
        camillaId,
        estado: { notIn: ["descartada", "discarded"] },
    };
}
async function assertCamillaCapacity(camillaId, adding = 1, ignoredPlantId) {
    const camilla = await prisma.camilla.findUnique({ where: { id: camillaId } });
    if (!camilla)
        throw new ApiError(404, "Camilla no encontrada.");
...
async function assertCamillaPosition(camillaId, posicionCamilla, ignoredPlantId) {
    const camilla = await prisma.camilla.findUnique({ where: { id: camillaId } });
    if (!camilla)
        throw new ApiError(404, "Camilla no encontrada.");
    ...
    if (existing)
        throw new ApiError(409, "Ya existe una planta en esa posicion de la camilla.");
}
function parseCamillaPayload(body, partial = false) {
...
function parsePlantaPayload(body, partial = false) {
    ...
    return Object.fromEntries(Object.entries(parsed).map(([key, value]) => [key, parseOptionalString(value)]).filter(([, value]) => value !== undefined));
}
function salaRoutes() {
    const router = Router();
    router.get("/", async (_req, res, next) => {
    ...
    router.delete("/:id", async (req, res, next) => {
        try {
            ...
            if (related.length > 0) {
                throw new ApiError(409, `No se puede eliminar la sala porque tiene datos relacionados: ${related.join(", ")}. Borra o reasigna esos datos antes de eliminarla.`);
            }
...
function camillaRoutes() {
    ...
    router.post("/", async (req, res, next) => {
        try {
            res.status(201).json(await prisma.camilla.create({ data: parseCamillaPayload(req.body) }));
        }
        catch (error) {
            next(error);
        }
    ...
    router.put("/:id", async (req, res, next) => {
        try {
            const id = parseId(req);
            const data = parseCamillaPayload(req.body, true);
            if (data.capacidadMaximaPlantas !== undefined) {
                const occupied = await prisma.planta.count({ where: activePlantWhereForCamilla(id) });
                if (data.capacidadMaximaPlantas < occupied)
                    throw new ApiError(400, "La capacidad no puede quedar por debajo de las plantas actuales.");
            }
    ...
    router.delete("/:id", async (req, res, next) => {
        try {
            ...
            if (related.length > 0) {
                throw new ApiError(409, `No se puede eliminar la camilla porque tiene datos relacionados: ${related.join(", ")}. Borra o reasigna esos datos antes de eliminarla.`);
            }
...
function plantaRoutes() {
    ...
    router.post("/", async (req, res, next) => {
        try {
            ...
            res.status(201).json(await prisma.planta.create({ data: data, include }));
        }
        catch (error) {
            next(error);
        }
    ...
    router.post("/bulk", async (req, res, next) => {
        try {
            ...
            if (!camilla)
                throw new ApiError(404, "Camilla no encontrada.");
            const occupied = new Set((await prisma.planta.findMany({
                where: activePlantWhereForCamilla(body.bedId),
                select: { posicionCamilla: true },
            })).map((plant) => plant.posicionCamilla));
            const freePositions = Array.from({ length: Math.min(camilla.capacidadMaximaPlantas, 100) }, (_, index) => index + 1).filter((position) => !occupied.has(position));
            if (freePositions.length < body.count)
                throw new ApiError(400, "No hay posiciones libres suficientes en la camilla.");
        ...
        try {
            ...
            res.json(await prisma.planta.update({
                ...
                data: {
                    ...
                    observaciones: parseOptionalString(req.body.observaciones ?? req.body.notes),
                },
                include,
            }));
        }
    ...
    router.get("/:id", async (req, res, next) => {
        try {
            ...
            if (!record)
                throw new ApiError(404, "Planta no encontrada.");
            res.json(record);
        }
        catch (error) {
            next(error);
    ...
    router.put("/:id", async (req, res, next) => {
        try {
            ...
            res.json(await prisma.planta.update({ where: { id }, data: data, include }));
        }
        catch (error) {
            next(error);
        }
...
function madreRoutes() {
    ...
    router.get("/:id", async (req, res, next) => {
        try {
            ...
            if (!record)
                throw new ApiError(404, "Madre no encontrada.");
            res.json(record);
        }
        catch (error) {
            next(error);
    ...
    router.delete("/:id", async (req, res, next) => {
        try {
            ...
            if (plantas > 0) {
                throw new ApiError(409, `No se puede eliminar la madre porque tiene ${plantas} planta${plantas === 1 ? "" : "s"} asociada${plantas === 1 ? "" : "s"}. Para conservar la trazabilidad, archivala o descartala, o reasigna esas plantas antes.`);
            }
...
function cosechaRoutes() {
    ...
    router.get("/:id", async (req, res, next) => {
        try {
            ...
            if (!record)
                throw new ApiError(404, "Cosecha no encontrada.");
            res.json(record);
        }
        catch (error) {
            next(error);
...
function registroAmbientalRoutes() {
    ...
    router.get("/:id", async (req, res, next) => {
        try {
            ...
            if (!record)
                throw new ApiError(404, "Registro ambiental no encontrado.");
...
function medicionRoutes() {
    ...
    router.get("/", async (req, res, next) => {
        try {
            const { salaCultivoId, camillaId, clonadorId, plantaId, madreId, estado, tipo, fechaDesde, fechaHasta } = req.query;
    ...
    router.get("/:id", async (req, res, next) => {
        try {
            ...
            if (!record)
                throw new ApiError(404, "Medicion no encontrada.");
            res.json(record);
        }
        catch (error) {
            next(error);
...
function clonadorRoutes() {
    ...
    router.get("/:id", async (req, res, next) => {
        try {
            ...
            if (!record)
                throw new ApiError(404, "Clonador no encontrado.");
            res.json(record);
        }
        catch (error) {
            next(error);
    ...
    router.delete("/:id", async (req, res, next) => {
        try {
            ...
            if (count > 0)
                throw new ApiError(409, `El clonador tiene ${count} esqueje(s) activo(s) y no puede eliminarse.`);
    ...
    router.post("/:id/bulk", async (req, res, next) => {
        try {
            ...
            if (!clonador)
                throw new ApiError(404, "Clonador no encontrado.");
            if (!genetica)
                throw new ApiError(400, `Genética con ID ${body.geneticsId} no existe. Verificá que esté guardada en la base de datos.`);
            if (body.motherPlantId) {
                const madre = await prisma.madre.findUnique({ where: { id: body.motherPlantId }, select: { id: true } });
                if (!madre)
                    throw new ApiError(400, `Madre con ID ${body.motherPlantId} no existe.`);
            }
            if (body.batchId) {
                const lote = await prisma.loteCultivo.findUnique({ where: { id: body.batchId }, select: { id: true } });
                if (!lote)
                    throw new ApiError(400, `Lote con ID ${body.batchId} no existe.`);
            }
            ...
            const occupiedSet = new Set(occupiedPositions.map((p) => p.posicionClonador).filter(Boolean));
            const freePositions = Array.from({ length: Math.min(clonador.capacidadMaximaEsquejes, 60) }, (_, i) => i + 1).filter((pos) => !occupiedSet.has(pos));
            if (freePositions.length < body.count) {
    ...
    router.post("/:id/send-to-camilla", async (req, res, next) => {
        try {
            const clonadorId = parseId(req);
            ...
            if (!camilla)
                throw new ApiError(404, "Camilla destino no encontrada.");
            const freeSlots = camilla.capacidadMaximaPlantas - camilla._count.plantas;
            if (plantIds.length > freeSlots)
                throw new ApiError(409, `La camilla destino solo tiene ${freeSlots} lugar(es) libre(s).`);
            ...
            for (const plantId of plantIds) {
                while (occupiedSet.has(nextPos))
                    nextPos++;
                assignments.push({ plantId, position: nextPos });
                occupiedSet.add(nextPos);
                nextPos++;
            }
            await Promise.all(assignments.map(({ plantId, position }) => prisma.planta.update({
                where: { id: plantId },
...
function sistemaRiegoRoutes() {
    ...
    router.get("/", async (req, res, next) => {
        try {
            ...
            res.json(await prisma.sistemaRiego.findMany({
    ...
    router.get("/:id", async (req, res, next) => {
        try {
            ...
            if (!record)
                throw new ApiError(404, "Sistema de riego no encontrado.");
            res.json(record);
        }
        catch (error) {
            next(error);
    ...
    router.put("/:id", async (req, res, next) => {
        try {
            res.json(await prisma.sistemaRiego.update({
                where: { id: parseId(req) },
...
cultivationRoutes.post("/vpd/preview", (req, res, next) => {
    try {
        const body = req.body;
        const vpd = envLogCalcVPD(Number(body.airTempC), Number(body.relativeHumidity), body.leafTempC != null && body.leafTempC !== "" ? Number(body.leafTempC) : undefined);
