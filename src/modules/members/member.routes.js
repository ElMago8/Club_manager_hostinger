...
const TIPOS_DOCUMENTO = ["credencial", "dni_frente", "dni_dorso", "reprocann", "certificado_medico", "autorizacion", "otro"];
...
function socioRoutes() {
    ...
    router.get("/", async (req, res, next) => {
        try {
            ...
            res.json(await prisma.socio.findMany({
                where,
    ...
    router.post("/", async (req, res, next) => {
        try {
            ...
            res.status(201).json(await prisma.socio.create({ data, include: SOCIO_INCLUDE }));
        }
        catch (error) {
            next(error);
        }
    ...
    router.patch("/:id", async (req, res, next) => {
        try {
            const data = buildSocioData(socioSchema.partial().parse(req.body));
            res.json(await prisma.socio.update({
                where: { id: parseId(req) },
                data,
                include: SOCIO_INCLUDE,
            }));
...
function documentoRoutes() {
    ...
    router.get("/", async (req, res, next) => {
        try {
            ...
            if (!socio)
                throw new ApiError(404, "Socio no encontrado.");
            res.json(await prisma.documentoSocio.findMany({
                where: { socioId },
                orderBy: [{ tipoDocumento: "asc" }, { fechaVencimiento: "desc" }],
            }));
        }
    ...
    router.post("/", uploadBySocio.single("arquivo"), async (req, res, next) => {
        try {
            const socioId = intId.parse(req.params.id);
            const socio = await prisma.socio.findUnique({ where: { id: socioId } });
            if (!socio)
            ...
            res.status(201).json(await prisma.documentoSocio.create({
                data: {
                    ...body,
                    socioId,
                    archivoUrl,
...
function documentoStandaloneRoutes() {
    ...
    router.patch("/:id", uploadByDoc.single("arquivo"), async (req, res, next) => {
        try {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            res.json(await prisma.documentoSocio.update({
                where: { id },
                data: { ...body, ...extra },
            }));
        }
