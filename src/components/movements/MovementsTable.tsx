...
export function MovementsTable({ movements, itemNameMap, locationNameMap }: MovementsTableProps) {
  ...
  if (sorted.length === 0) {
    return <p className="py-16 text-center text-sm text-muted-foreground">Sin movimientos registrados</p>;
  }

  const pagination = (
    <div className="mt-3 flex items-center justify-between text-sm text-muted-foreground">
      <span>Mostrando {start}–{end} de {sorted.length} movimientos</span>
      <div className="flex gap-2">
        <Button variant="outline" size="sm" disabled={safePage === 0} onClick={() => setPage(safePage - 1)}>Anterior</Button>
        <Button variant="outline" size="sm" disabled={safePage >= totalPages - 1} onClick={() => setPage(safePage + 1)}>Siguiente</Button>
      </div>
    </div>
  );
              ...
              <Card key={m.id} className="cursor-pointer" onClick={() => setExpandedId(expandedId === m.id ? null : m.id)}>
                ...
                <CardContent className="px-4 pb-3 space-y-1 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="truncate ml-2 font-medium">{itemNameMap.get(m.itemId) ?? "Sin datos"}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  {expandedId === m.id && (
                    <div className="pt-2 border-t border-border mt-2 space-y-1">
                      {m.reference && <div><span className="text-muted-foreground">Motivo:</span> {m.reference}</div>}
                      {m.notes && <div><span className="text-muted-foreground">Observación:</span> {m.notes}</div>}
          ...
          <Table>
            <TableHeader className="sticky top-0 bg-card">
              <TableRow>
                ...
                <TableHead className="w-[80px]">Sentido</TableHead>
                ...
                return (
                  <Fragment key={m.id}>
                    <TableRow className="cursor-pointer hover:bg-muted/50" onClick={() => setExpandedId(isExpanded ? null : m.id)}>
                      ...
                      <TableCell className="font-medium">{itemNameMap.get(m.itemId) ?? <span className="italic text-muted-foreground/60 line-through">[Producto eliminado]</span>}</TableCell>
                      <TableCell>
                        <span className={`font-mono text-sm font-medium ${dir === "in" ? "text-emerald-600" : "text-red-500"}`}>{dir === "in" ? "+" : "−"}{absQty}</span>
                      </TableCell>
                      <TableCell>
                        <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium ${dir === "in" ? "bg-emerald-500/10 text-emerald-600" : "bg-red-500/10 text-red-500"}`}>{dir === "in" ? "entrada" : "salida"}</span>
                      </TableCell>
  ...
  return (
    <div className="space-y-2 text-sm">
      {(movement.notes || movement.reference) && (
        <div><span className="font-medium text-foreground">Observación: </span><span className="text-muted-foreground">{movement.notes || movement.reference}</span></div>
      )}
      {isTransfer && (fromLocation || toLocation) && (
        <div><span className="font-medium text-foreground">Traslado: </span><span className="text-muted-foreground">{fromLocation ?? "—"} → {toLocation ?? "—"}</span></div>
      )}
      <div>
