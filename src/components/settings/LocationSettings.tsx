...
export function LocationSettings() {
  ...
  if (tree.length === 0 && addingParentId !== "__root__") {
    return <EmptyState icon={Warehouse} title="Sin ubicaciones configuradas" description="Construí tu jerarquía de ubicaciones empezando por un espacio principal." actionLabel="Agregar espacio" onAction={handleAddRoot} />;
  }

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">{tree.length} ubicación{tree.length !== 1 && "es"} raíz</p>
        <Button size="sm" variant="outline" onClick={handleAddRoot}>
          <Plus className="mr-1.5 h-3.5 w-3.5" /> Agregar espacio
        </Button>
      </div>
      ...
      {addingParentId === "__root__" && (
        <div className="flex items-center gap-1 rounded-lg border border-border p-3">
          <Input ref={addRef} value={newName} onChange={(e) => setNewName(e.target.value)}
            ...
            placeholder="Nombre del espacio…" className="h-8 text-sm" />
