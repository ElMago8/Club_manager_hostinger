...
export const Route = createFileRoute("/app/movements")({
  ...
  head: () => ({ meta: [{ title: "Movimientos · Cannabis Club Manager" }] }),
...
function MovementsPage() {
  ...
  const movementCsvColumns = useMemo<CSVColumn<StockMovement>[]>(() => [
    { header: "Fecha", accessor: (m) => new Date(m.createdAt).toLocaleDateString() },
    { header: "Tipo", accessor: (m) => m.type },
    { header: "Producto", accessor: (m) => itemNameMap.get(m.itemId) ?? "" },
    { header: "Lote", accessor: (m) => items.find((i) => i.id === m.itemId)?.sku ?? "" },
    { header: "Cantidad", accessor: (m) => m.quantity },
    { header: "Responsable", accessor: (m) => m.performedBy },
    { header: "Motivo", accessor: (m) => m.reference },
    { header: "Observación", accessor: (m) => m.notes },
  ], [itemNameMap, items]);
    ...
    <div className="mx-auto max-w-[1400px] space-y-6">
      ...
      {movements.length === 0 ? (
        <EmptyState
          ...
          description="Los movimientos registran cosechas, dispensas, ajustes y mermas del club."
