...
// `DemoUser` se mantiene por compatibilidad con UserManagement existente.
// El dominio Cannabis Club Manager usa `AppUser` (en src/types/inventory.ts).
...
const SEED_USERS: DemoUser[] = [
  { id: "user-01", name: "Admin Club", email: "admin.club@hipnosis-demo.local", role: "admin", status: "active", joinedAt: new Date(Date.now() - 180 * 86400000).toISOString() },
  { id: "user-02", name: "Lucía Operadora", email: "op.lucia@hipnosis-demo.local", role: "manager", status: "active", joinedAt: new Date(Date.now() - 120 * 86400000).toISOString() },
  { id: "user-03", name: "Matías Operador", email: "op.matias@hipnosis-demo.local", role: "manager", status: "active", joinedAt: new Date(Date.now() - 90 * 86400000).toISOString() },
  { id: "user-04", name: "Romina Operadora", email: "op.romina@hipnosis-demo.local", role: "manager", status: "active", joinedAt: new Date(Date.now() - 60 * 86400000).toISOString() },
  { id: "user-05", name: "Auditoría Club", email: "audit.club@hipnosis-demo.local", role: "requestor", status: "active", joinedAt: new Date(Date.now() - 45 * 86400000).toISOString() },
];
...
export class DemoStore {
  ...
  getMemberById(id: string): Member | undefined { return this.data.members.find((m) => m.id === id); }
  createMember(member: Member): Member { this.data.members.push(member); this.version++; return member; }
  ...
  deleteMember(id: string): boolean {
    ...
    if (this.data.members.length < len) { this.version++; return true; }
    return false;
  }
  ...
  addAuditEntry(entry: AuditEntry): void { this.data.audit.push(entry); this.version++; }
