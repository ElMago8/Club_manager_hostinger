...
const invoiceTypes = ["factura_c", "nota_credito_c", "nota_debito_c", "recibo_interno"];
const arcaStatuses = ["aprobado", "pendiente", "observado", "rechazado"];
const paymentStatuses = ["pagado", "impago", "parcial", "vencido"];
...
const invoiceSchema = rawInvoiceSchema.transform(normalizeInvoiceBody).superRefine((body, ctx) => {
    if (!body.socioId && !body.razonSocial)
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Indica un socio o una razon social manual.", path: ["razon_social"] });
    if (!body.tipoComprobante)
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "tipo_comprobante es requerido.", path: ["tipo_comprobante"] });
    if (!body.fechaEmision)
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "fecha_emision es requerida.", path: ["fecha_emision"] });
    if (!body.concepto)
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "concepto es requerido.", path: ["concepto"] });
    if (body.total === undefined || body.total === 0)
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "El total no puede ser 0.", path: ["total"] });
});
...
function prefixForType(type) {
    if (type === "nota_credito_c")
        return "NC";
    if (type === "nota_debito_c")
        return "ND";
    if (type === "recibo_interno")
        return "REC";
    return "FAC";
}
...
async function buildCodes(type, pointOfSale = "0001") {
...
function invoiceDataFromBody(body, socio) {
    if (body.socioId && !socio)
        throw new ApiError(404, "Socio no encontrado.");
...
billingRoutes.get("/invoices", async (_req, res, next) => {
    try {
        res.json(await prisma.comprobanteFacturacion.findMany({
...
billingRoutes.get("/invoices/:id", async (req, res, next) => {
    try {
...
billingRoutes.post("/invoices", async (req, res, next) => {
    try {
...
billingRoutes.patch("/invoices/:id", async (req, res, next) => {
    try {
...
billingRoutes.delete("/invoices/:id", async (req, res, next) => {
    try {
        const id = parseId(req);
        res.json(await prisma.comprobanteFacturacion.update({
            where: { id },
...
billingRoutes.post("/invoices/:id/mark-paid", async (req, res, next) => {
    try {
        const id = parseId(req);
...
async function createRelatedNote(id, noteType, body) {
...
billingRoutes.post("/invoices/:id/credit-note", async (req, res, next) => {
    try {
...
billingRoutes.post("/invoices/:id/debit-note", async (req, res, next) => {
    try {
